...
const date = new Date();
const filename = `./${date.getFullYear()} ${date.getMonth()} ${date.getDate()} ${date.getHours() < 10 ? '0' + date.getHours() : date.getHours()}-${date.getMinutes() < 10 ? '0' + date.getMinutes() : date.getMinutes()}-${date.getSeconds()}.log`
// fs.appendFileSync(filename, '')
log4js.configure({
    appenders: {
        // 配置一个 default 对象, type代表输出文件类型 filename代表输出文件名
        default: { type: 'file', filename },
