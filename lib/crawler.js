...
class Crawler {
	 ...
	 */
	async downloadFromPackage (packageJson, command) {
		const dependencies = packageJson.dependencies;
		if (command.devDependencies) {
			Object.assign(dependencies, command.devDependencies)
		}
		if (command.peerDependencies) {
			Object.assign(dependencies, command.peerDependencies)
		}
		const tarballArr = [];
		for (const dependenciesKey in dependencies) {
			const dependency = dependencies[dependenciesKey];
			const tarballsSet = await this.getDependencies({name: dependenciesKey, version: dependency, command, undefined, isMoreVersion: false})
