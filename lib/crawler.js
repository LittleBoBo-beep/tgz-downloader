...
class Crawler {
	...
	/**
	 * 获取dependencies
	 * @param { {name: string, version: string, devDependencies?: boolean, peerDependencies?: boolean, registry?: string, isMoreVersion?: boolean, devDependencies?: boolean, peerDependencies?: boolean, outputPrefix?: ''} }options
	 * @return {Set<string>}
	 */
	async getDependencies(options) {
		const {name, version, isMoreVersion = false} = options;
	 ...
	 * @param packageLock
	 */
	async downloadFromPackageLock (packageLock) {
	...
	/**
	 *
	 * @param {Object.<string, string | object>} dependenciesObject
	 * @param outputPrefix
	 */
	async getDependenciesFrom (dependenciesObject, outputPrefix) {
		await Promise.all(Object.entries(dependenciesObject).map(async ([name, version]) => {
			...
			return this.getDependencies({name, version, outputPrefix});
		}))
	}
