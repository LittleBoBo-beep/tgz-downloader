...
class Crawler {
	...
	async retrievePackageVersion({ name, version, outputPrefix = '', command, registry }) {
		...
		if (this.packagesCache.has(name)) {
			...
			this.cacheHits++;
			// const allPackageVersionsDetails = ;
			// return allPackageVersionsDetails;
			return Promise.resolve(this.packagesCache.get(name))
			// const maxSatisfyingVersion = this.getMaxSatisfyingVersion(allPackageVersionsDetails, version);
			// return Promise.resolve(allPackageVersionsDetails.versions[maxSatisfyingVersion]);
		}
		...
		return Promise.resolve(allPackageVersionsDetails)
		// const maxSatisfyingVersion = this.getMaxSatisfyingVersion(allPackageVersionsDetails, version);
		// return Promise.resolve(allPackageVersionsDetails.versions[maxSatisfyingVersion]);
	}
