...
class Crawler {
	...
	async getDependencies(options) {
		...
		for (let i = 0, length = lists.length; i < length; i++) {
			...
			this.tarballs.add({url: packageJson.dist.tarball, path: name})
			if (dependencies) {
	...
	cachePackageFromPackages (dependencies, registry) {
	...
	async getDependenciesFrom (dependenciesObject, outputPrefix) {
		return await Promise.all(Object.entries(dependenciesObject).map(([name, version]) => {
