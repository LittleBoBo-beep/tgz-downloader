...
class Crawler {
	...
	/**
	 * 获取dependencies
	 * @param {{isMoreVersion: boolean, name: string, version: number, devDependencies?: boolean, peerDependencies?: boolean}}options
	 * @return {Set<string>}
	 */
	...
	async getDependenciesFrom (dependenciesObject, outputPrefix) {
		await Promise.all(Object.entries(dependenciesObject).map(async ([name, version]) => {
			...
			return this.getDependencies({name, version, isMoreVersion: false});
		}))
	}
