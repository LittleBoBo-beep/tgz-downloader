...
class Downloader {
    ...
    downloadTarballs(tarballs, baseDirectory = './tarballs', concurrency = Infinity, packagesCache) {
        ...
        return import('p-limit').then(({default: pLimit}) => {
            ...
            for (let i = 0, length = values.length; i < length; i++) {
                ...
                logger('debug', ['downloading'.cyan, position], keys[i]);
                const jsonPath = baseDirectory + '/' + keys[i];
                promises.push(limit(() => this.requestPackageJson(values[i], this.TIME_OUT, jsonPath, 2, position, keys[i])));
            }
    ...
    requestPackageJson(uri, timeout, jsonPath, maxCount = 2, position, name) {
        // jsonPath += ;
        ...
        return axios.request({url: uri, responseType: 'json', timeout})
            ...
            .catch(error => {
            logger('error', ['failed download packageJson'.red], error.message, (maxCount - 1 + '').green);
            if (maxCount > 0) {
                maxCount--;
