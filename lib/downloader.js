...
class Downloader {
	...
	async downloadFromIterable (tarballsIterable, options, packagesCache) {
		// const { tarballsSet: tarballs, packagesCache, baseDirectory } = options;
	...
	async downloadFileWithRetry (url, directory, position, count, packageJson) {
		try {
			// 开始下载tgz文件和package文件
			const { path, duration } = await this.downloadFileAsync(url, { directory }, packageJson);
			if (!existsSync(path)) {
			...
			if (count > 0) await this.downloadFileWithRetry(url, directory, position, count - 1);
		}
	}
