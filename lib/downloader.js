...
class Downloader {
	...
	downloadTarballs (tarballs, baseDirectory = './tarballs', concurrency = Infinity, packagesCache) {
		...
		logger('debug', [`downloading tarballs (concurrency: ${concurrency})`.bgGreen], { count: tarballs.length });
	...
	async downloadFileWithRetry (url, directory, position, count, packageJson) {
		try {
			const duration = 1000;
			// 开始下载tgz文件和package文件
			const { path } = await this.downloadFileAsync(url, { directory, duration }, packageJson);
			if (!existsSync(path)) {
				new Error(`tgz does not exist ${path}`);
			}
			if (this.validateTarball(path)) logger('debug', ['downloaded tgz'.green, position], url, `${duration}ms`.gray);
			else {
			...
			return req.get(
				...
				if (response.status === 200) {
					mkdirp(options.directory, (error) => {
						...
						// 设置延时器，增加容错率
						// setTimeout(() => {
						if (this.isHttpUrl(packageJson)) {
							this.requestPackageJson(packageJson, options.timeout).then(({data: packageJson}) => {
	...
	requestPackageJson(uri, timeout) {
		return axios.request({ method: 'GET', url: uri, responseType: 'json', timeout }).catch(error => {
			logger('error', ['failed download packageJson'.red], error.message);
		})
	}
	isHttpUrl(str) {
